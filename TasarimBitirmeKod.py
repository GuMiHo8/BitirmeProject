...
def AgIleAyirma():
    ...
    egitim = cv.dnn.readNet("sekli_ayirma.cfg","sekli_ayirma.weights") # eğittilen şekil tanıma ağı

    # nesnenin adı
    classes = ["sekil"]

    images_path = glob.glob("Soru.jpg")                                 # taranacak resimlerin yolunu belirtme
    ...
    for img_path in images_path:
        ...
        for out in outs:
            for detection in out:
                ...
                if confidence > 0.3:
                    ...
                    # oluşturduğumuz rectangle koordinatları
                    x = int(center_x - w / 2)
                    y = int(center_y - h / 2)
        ...
        for i in range(len(boxes)):
            if i in indexes:
                x, y, w, h = boxes[i]
                label = str(classes[class_ids[i]])   # labelları tanımlamak için
                img[y:y+h,x:x+w] = (255,255,255)
                cv.imwrite('metin1.jpg',img)        # metin görüntüsü 
            
        cv.imshow("Soru ", img)
        #print("Görüntüden Metin Ayıklama Yapıldı.")
        key = cv.waitKey(0)
...
def MouseIleAyirma():
    ...
    k = False
    x1, y1, x2, y2,  = 0, 0, 0, 0,
    goruntu = cv.imread('9.jpg')
    img = goruntu.copy() # görüntüde 2 faklı metin alanı var diye kopyasını alalım 

    def kirpma(simdi, x, y, t, z):
    
        global x1, y1, x2, y2, k


        if simdi == cv.EVENT_LBUTTONDOWN: # faresin sol tuşuna tıklanınca

            x1, y1, x2, y2 = x, y, x, y   # ilk tıklamanın olduğu konumun değerlerini atama
            k = True
    
        elif simdi == cv.EVENT_MOUSEMOVE: # fareyi hareket ettirinceki
            if k == True:
                x2, y2 = x, y             # hareketin son konumundaki değeri atama
   
        elif simdi == cv.EVENT_LBUTTONUP: # sol tuş tıklama bırakılınca
        
            x2, y2 = x, y                 # farenin son bulunduğu konumun x ve y değerini atama
            k = False 
            k_alan = [(x1, y1), (x2, y2)]

            if len(k_alan) == 2: # tüm alan seçilmişse 

                metin = img[k_alan[0][1]:k_alan[1][1], k_alan[0][0]:k_alan[1][0]] 
                cv.imshow("Metin", metin)
                cv.imwrite('metin.jpg',metin)

    cv.namedWindow("goruntu")
    cv.setMouseCallback("goruntu", kirpma)
    while True:
        ...
        if not k:
            ...
            cv.rectangle(i, (x1, y1), (x2, y2), (200,61,200), 2)
            cv.imshow("goruntu", i)
...
def metinOku(resim_yolu):
    ...
    kernel = np.ones((1,1), np.uint8)#yazı biçimine göre ayarlanıyo
...
def Analiz(soru):
    ...
    #soru = "ABC bir dik üçgendir. |AB|=2|BC|,|AB| = 2 ise |AC|=? ,|CD| =2x-10, m(ABC)=100, m(ABC)=2m(ABC), m(BCD)=x kaçtır?" # örnek soru girdisi
            ...
            while i < len(KorA):
                ...
                while j < len(KorA):
                    if (KorA[i][3] == KorA[j][3] and KorA[i][1] != "s" and KorA[j][1] == "s"):
                        KorA[j][1] = int(KorA[i][1]) * float(KorA[i][2]/KorA[j][2])
                        OranYazdir(KorA[i],KorA[j])
                    
                    if (KorA[i][3] == KorA[j][3] and KorA[i][1] == "s" and KorA[j][1] != "s"):
                        KorA[i][1] = int(KorA[j][1]) * float(KorA[j][2]/KorA[i][2])
...
def FonkCagır():
    #AgIleAyirma()
    soru = metinOku('metin1.jpg')
    #print("****************************** GÖRÜNTÜDEN GELEN METİN ***************************************")
    print("****************************** GÖRÜNTÜDEN AGILANAN METİN **************************************")
    print(soru)
    print("***************************** 1. KISIMDAN GELEN BİLGİLER ************************************")
    print("")
    Analiz(soru)
    #MouseIleAyirma() # def içine aldığımdan hata veriyor nedenini bul düzenle
    #met_gor = cv.imread('metin.jpg')
